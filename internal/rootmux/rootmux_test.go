...
func createUnLoggedTests(t *testing.T) func(wg *sync.WaitGroup) {
	...
	return func(wg *sync.WaitGroup) {
		...
		// Try to access a non crypted file on an encrypted unsecured dav (must fail)
		do("GET", "unsecureddav.vestibule.io/mydata/test.txt", noH, "", 500, "cipher: message authentication failed")
		// Try to access a crypted file on a encrypted unsecured dav (must pass)
		do("GET", "unsecureddav.vestibule.io/mydata/test-ciphered.txt", noH, "", 200, "content is encrypted !")
...
func createUserTests(t *testing.T) func(wg *sync.WaitGroup) {
	...
	return func(wg *sync.WaitGroup) {
		...
		tests := func() {
			...
			do("GET", "/api/common/davs", xsrfHeader, "", 200, `[{"id":1,"host":"unsecureddav.vestibule.io","root":"./testdata/data","writable":true,"secured":false,"usedgb":`)
...
func createAdminTests(t *testing.T) func(wg *sync.WaitGroup) {
	...
	return func(wg *sync.WaitGroup) {
		...
		tests := func() {
			...
			do("GET", "/api/admin/davs/", xsrfHeader, "", 200, `[{"id":1,"host":"unsecureddav.vestibule.io","root":"./testdata/data","writable":true,"secured":false,"passphrase":"very secret passphrase"`)
